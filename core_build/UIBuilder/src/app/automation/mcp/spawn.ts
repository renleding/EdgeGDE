...
export async function spawnMCPIfNeeded(): Promise<AutomationServerHandle | null> {
  // Bypass: always attempt to connect to an existing MCP server.
  // In dev/browser mode we skip the Tauri spawn path.
  if (!isTauri()) {
    ...
    // No server found in browser mode — return null; the caller's
    // connectAutomation will poll until a server appears.
    return null
  }

  // Bypass: if an MCP server is already running on the host (started
  // by Hermes startup scripts or manual launch), connect to it instead
  // of spawning a new one — avoids port conflicts and dependency on the
  // Tauri shell plugin.
  const existing = await readHealth()
  if (existing) {
    assertCompatibleMcpVersion(existing)
