...
/// Proxy an HTTP request from the Rust backend, bypassing the Tauri webview's
/// fetch() which can strip Authorization headers on cross-origin requests.
#[tauri::command]
async fn proxy_http_request(
    ...
    for header_pair in &headers {
        if header_pair.len() >= 2 {
            req = req.header(&header_pair[0], &header_pair[1]);
        }
    }
    ...
    let resp_headers: Vec<Vec<String>> = resp
        .headers()
        .iter()
