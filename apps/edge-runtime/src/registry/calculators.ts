...
/**
 * Hydrate CALCULATOR_REGISTRY from KV store on startup.
 * Merges KV-backed calculators into the registry, preserving built-in tools.
 * KV-backed calculators override built-in ones with the same ID.
 */
export async function hydrateCalculatorsFromKV(kv: KvStore): Promise<void> {
  const { keys } = await kv.list('calc:')
  const processed = new Set<string>()

  for (const key of keys) {
    // Only process "calc:{id}:latest" keys
    const match = key.name.match(/^calc:(.+):latest$/)
    if (!match) continue

    const calcId = match[1]
    if (processed.has(calcId)) continue
    processed.add(calcId)
    ...
    try {
      ...
      const artifactKey = `calc:${calcId}:${version}`
      const artifactData = await kv.get(artifactKey)
      if (!artifactData) continue

      const artifact: DesignArtifact = JSON.parse(artifactData)
      if (artifact.type !== 'calculator') continue

      // Build a simple execution stub for published calculators
      // Use z.object({}) as a passthrough for dynamic schemas
      const schema = z.object({})

      CALCULATOR_REGISTRY[calcId] = {
        id: calcId,
        description: `Published calculator: ${calcId} (v${version})`,
        schema: schema as any,
        layout: artifact.layout as LayoutDefinition,
        execute(input: any): any {
          // For published calculators, return the input as output
          // Actual execution logic will be added in future phases
          return { ...input, publishedVersion: version }
        },
      }
    } catch {
