...
'<style>' +
':root{--bg:#0a0a0b;--fg:#e4e4e7;--border:#2a2a2e;--hover:#141416;--muted:#71717a;--accent:#3b82f6;--success:#22c55e;--danger:#ef4444;--warning:#eab308;--surface:#141416;--surface-alt:#1c212e;--ring-fg:#3fb950;--ring-bg:#2d3140;--loss-border:#f85149;--loss-bg:#2d1b1b;--loss-text:#ff7b72;--btn-primary:#238636;--spacing-xs:4px;--spacing-sm:8px;--spacing-md:12px;--spacing-lg:16px;--spacing-xl:24px;--font-sm:12px;--font-md:13px;--font-lg:14px}' +
'*{margin:0;padding:0;box-sizing:border-box}' +
...
'.sug{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px;padding:0 12px 12px}' +
'.sug span{padding:3px 10px;border-radius:12px;font-size:11px;background:#1c212e;color:#8b949e;border:1px solid #2d3140;cursor:pointer}' +
'.sug span:hover{border-color:#3fb950;color:#3fb950;background:#162a16}' +
'.st{font-size:11px;color:#6e7681;margin:8px 12px 0;padding-bottom:4px;border-bottom:1px solid #2d3140}' +
'.lf{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:#00000080;align-items:center;justify-content:center;z-index:1000}' +
'.lf-c{background:#1c2128;border:2px solid #f85149;border-radius:12px;padding:20px;max-width:400px;width:90%}' +
'.lf-h{font-size:14px;font-weight:600;color:#ff7b72;margin-bottom:12px}' +
'.lf-i{font-size:12px;color:#8b949e;margin-bottom:16px;padding:8px 12px;background:#2d1b1b;border-radius:6px;border:1px solid #f85149}' +
'.lf-b{display:flex;gap:8px;justify-content:flex-end}' +
'.lf-b button{padding:8px 16px;border-radius:6px;border:none;font-size:12px;cursor:pointer}' +
'.lf-r{padding:6px 14px;border-radius:6px;border:1px solid #f85149;font-size:12px;cursor:pointer;background:#0a0a0b;color:#ff7b72}' +
'.lf-r:hover{background:#2d1b1b}' +
'</style></head><body>' +
'<div class="lf" id="lf"><div class="lf-c">' +
'<div class="lf-h">&#9888; You&#39;re about to lose data</div>' +
'<div class="lf-i" id="lf-items"></div>' +
'<div class="lf-b"><button class="lf-r" id="lf-dismiss">I&#39;ll risk it</button><button id="lf-keep" style="background:#238636;color:#fff">Keep it</button></div></div></div>' +
...
'<button id="afb">+ Add Field</button></div>' +
'<div class="st" id="st">Suggested fields</div>' +
'<div class="sug" id="sug"></div></div>' +
'<script>' +
'var E=null;' +
'function addF(){var d=document.getElementById("ad").value;var n=document.getElementById("afn").value;var v=document.getElementById("afv").value;if(!n||!v)return;fetch("/api/v1/doc-intel/documents/custom-fields",{method:"POST",headers:{"Content-Type":"application/json","x-tenant":"personal"},body:JSON.stringify({document_id:d,field_name:n,field_value:"MANUAL_OVERRIDE:"+v})}).then(function(r){if(r.ok)location.reload();else alert("Add field failed: "+r.status)})["catch"](function(e){alert("Add field error: "+e)})}' +
'var sugMap={licence:["licence_number","full_name","address","date_of_birth","date_of_expiry","state","licence_class","card_number"],passport:["passport_number","full_name","nationality","date_of_birth","date_of_expiry","place_of_issue"],medicare:["medicare_number","full_name","card_color","date_of_birth"],unknown:["name","company","membership_number","date","notes","reference","phone","email"]};' +
'function updSug(){var sel=document.getElementById("ad");var opt=sel&&sel.options[sel.selectedIndex];if(!opt)return;var dt=opt.text.split("_")[0].toLowerCase()||"unknown";var sug=sugMap[dt]||sugMap.unknown;document.getElementById("sug").innerHTML=sug.map(function(f){return"<span data-field=\'"+f+"\'>"+f+"</span>"}).join("")}' +
'document.getElementById("ad").onchange=updSug;' +
'document.getElementById("lf-keep").onclick=function(){document.getElementById("lf").style.display="none"};' +
'window.addEventListener("click",function(e){var sf=e.target.closest("[data-field]");if(sf){document.getElementById("afn").value=sf.getAttribute("data-field");document.getElementById("afv").focus();return}if(e.target.id=="afb"){addF();return}var x=e.target.closest("[data-del]");if(x&&x.getAttribute("data-del")){document.getElementById("lf-items").textContent="Field: \\\\""+x.getAttribute("data-fname")+"\\\\"";document.getElementById("lf").style.display="flex";document.getElementById("lf-dismiss").onclick=function(){document.getElementById("lf").style.display="none";fetch("/api/v1/doc-intel/fields/"+encodeURIComponent(x.getAttribute("data-del"))+"/"+encodeURIComponent(x.getAttribute("data-fname")),{method:"DELETE",headers:{"x-tenant":"personal"}}).then(function(r){if(r.ok)location.reload();else alert("Delete failed: "+r.status)})["catch"](function(){alert("Network error")})};return}var ed=e.target.closest(".ed");if(ed&&!x){E={d:ed.getAttribute("data-docid"),f:ed.getAttribute("data-fname")};var nc=document.getElementById("fn-"+E.d+"-"+E.f);var vc=document.getElementById("v-"+E.d+"-"+E.f);E.o=E.f;var curVal=vc.textContent;nc.innerHTML="";var ni=document.createElement("input");ni.className="ei";ni.style.width="140px";ni.value=E.f;nc.appendChild(ni);vc.innerHTML="";var vi=document.createElement("input");vi.className="ei";vi.style.width="100%";vi.value=curVal;vc.appendChild(vi);var sb=document.createElement("span");sb.className="es";sb.setAttribute("data-act","sv");sb.textContent="save";vc.appendChild(sb);var cb=document.createElement("span");cb.className="ec";cb.setAttribute("data-act","cl");cb.textContent="x";vc.appendChild(cb);E.ni=ni;E.vi=vi;ni.focus()};var sv=e.target.closest("[data-act=sv]");if(sv&&E){var nn=(E.ni&&E.ni.value)||E.f;var nv=(E.vi&&E.vi.value)||"";if(!nv&&nv!=="")return;fetch("/api/v1/doc-intel/fields/"+encodeURIComponent(E.d)+"/"+encodeURIComponent(E.o||E.f),{method:"PUT",headers:{"Content-Type":"application/json","x-tenant":"personal"},body:JSON.stringify({value:nv,field_name:nn})}).then(function(r){if(r.ok)location.reload();else alert("Save failed: "+r.status)})["catch"](function(){alert("Save network error")})}var cl=e.target.closest("[data-act=cl]");if(cl&&E){var nc2=document.getElementById("fn-"+E.d+"-"+E.f);var vc2=document.getElementById("v-"+E.d+"-"+E.f);nc2.innerHTML=E.o||E.f;vc2.innerHTML=""}})' +
'</script></body></html>'
}
...
'#pb{height:3px;background:#2d3140;border-radius:2px;overflow:hidden;display:none;margin-top:8px}' +
'#pb-bar{height:100%;width:20%;background:#3fb950;border-radius:2px;animation:pb 1.5s ease-in-out infinite}' +
'@keyframes pb{0%{width:20%}50%{width:60%}100%{width:20%}}' +
'.lf{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:#00000080;align-items:center;justify-content:center;z-index:1000}' +
'.lf-c{background:#1c2128;border:2px solid #f85149;border-radius:12px;padding:20px;max-width:400px;width:90%}' +
'.lf-h{font-size:14px;font-weight:600;color:#ff7b72;margin-bottom:12px}' +
'.lf-i{font-size:12px;color:#8b949e;margin-bottom:16px;padding:8px 12px;background:#2d1b1b;border-radius:6px;border:1px solid #f85149}' +
...
'var DL_target=null;' +
'function DL(i){var doc=D.find(function(x){return x.document_id===i});var name=doc?doc.filename_display||"document":"document";document.getElementById("lf-items").innerHTML="Document: <strong>"+name+"</strong><br>Field data will be permanently lost.";DL_target=i;document.getElementById("lf").style.display="flex"}' +
'function DL_confirm(){var i=DL_target;if(!i)return;document.getElementById("lf").style.display="none";DL_target=null;fetch("/api/v1/doc-intel/documents/"+i,{method:"DELETE",headers:{"x-tenant":"personal"}}).then(function(r){if(!r.ok)throw Error("Delete failed");document.getElementById("p").style.display="none";L()}).catch(function(e){alert("Failed to delete: "+e.message)})}' +
'function up(f,cb){var pb=document.getElementById("pb");pb.style.display="block";var fd=new FormData();fd.append("file",f);fetch("/api/v1/doc-intel/ingest",{method:"POST",headers:{"x-tenant":"personal"},body:fd}).then(function(r){pb.style.display="none";if(r.ok&&cb)cb()}).catch(function(){pb.style.display="none"})}' +
...
'document.getElementById("lf-dismiss").onclick=DL_confirm;' +
'document.getElementById("lf-keep").onclick=function(){document.getElementById("lf").style.display="none";DL_target=null};' +
