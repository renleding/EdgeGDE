  ...
  if (sessionId) {
    try {
      const existing = await (c.env as any)?.DB?.prepare(
        `SELECT collected_fields_json FROM chat_sessions WHERE id = ? AND tenant_id = ?`
      ).bind(sessionId, c.req.query('tenant') || 'afirmico').first()
