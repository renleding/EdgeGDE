...
describe('AgenticMissionManifestSchema', () => {
  it('accepts valid typed mission', () => {
  ...
  it('rejects dependency cycles', () => {
    ...
    expect(JSON.stringify(result.error?.issues)).toContain('dependency cycles')
  })

  it('requires compensation and verification per step', () => {
    const result = AgenticMissionManifestSchema.safeParse(
      makeMission({
        compensationPlan: [],
      }),
    ...
    expect(result.success).toBe(false)
    expect(JSON.stringify(result.error?.issues)).toContain('compensationPlan')
  })
})

describe('LLMProposalSchema', () => {
  it('accepts non-executable proposal', () => {
  ...
  it('rejects executable action fields', () => {
    const result = LLMProposalSchema.safeParse({
      sessionId: 'session-1',
      tenantId: 'tenant-1',
      intent: 'Add calculator',
    ...
    expect(result.success).toBe(false)
    expect(JSON.stringify(result.error?.issues)).toContain('executable action fields')
  })
})

describe('assertLlmProposalIsNotExecutable', () => {
  it('LLM proposal is not executable as EdgeGDEAction', () => {
    ...
    expect(assertLlmProposalIsNotExecutable(proposal)).toBe(true)
  })
})

describe('EdgeGDEActionSchema', () => {
  it('requires idempotencyKey', () => {
    const result = EdgeGDEActionSchema.safeParse(makeAction({ idempotencyKey: undefined }))
    expect(result.success).toBe(false)
    expect(JSON.stringify(result.error?.issues)).toContain('idempotencyKey')
  })

  it('rejects non-sha256 idempotencyKey', () => {
    const result = EdgeGDEActionSchema.safeParse(makeAction({ idempotencyKey: 'not-a-sha256' }))
    expect(result.success).toBe(false)
    expect(JSON.stringify(result.error?.issues)).toContain('sha256')
  })

  it('rejects execute action requiring approval without approvalMode', () => {
    ...
    expect(result.success).toBe(false)
    expect(JSON.stringify(result.error?.issues)).toContain('approvalMode')
  })
})

describe('EdgeGDEActionResultSchema', () => {
  it('accepts normal execution result', () => {
...
describe('DryRunResultSchema', () => {
  it('rejects state-mutating dry run', () => {
    ...
    expect(result.success).toBe(false)
    expect(JSON.stringify(result.error?.issues)).toContain('wouldMutate')
  })

  it('rejects execute-mode dry run', () => {
    const result = DryRunResultSchema.safeParse(
      makeActionResult({
        executionMode: 'execute',
        wouldMutate: false,
    ...
    expect(result.success).toBe(false)
    expect(JSON.stringify(result.error?.issues)).toContain('executionMode')
  })
})

describe('CompensationActionSchema', () => {
  it('requires typed, idempotent compensation action', () => {
  ...
  it('rejects non-idempotent compensation action', () => {
    const result = CompensationActionSchema.safeParse({
      id: 'comp-1',
      originalActionId: 'action-1',
      sessionId: 'session-1',
...
describe('Execution order invariants', () => {
  it('rejects skipped dry-run', () => {
    ...
    expect(result.errors.some((error) => error.includes('dry_run'))).toBeTruthy()
  })

  it('accepts mandatory order', () => {
    const transitions = [
      { transition: 'validate', sequence: 0, auditEventId: 'audit-1', occurredAt: now },
      { transition: 'idempotency_check', sequence: 1, auditEventId: 'audit-2', occurredAt: now },
...
describe('UXEvent ordering', () => {
  it('must be strictly increasing', () => {
    ...
    expect(result.passed).toBe(false)
    expect(result.errors.some((error) => error.includes('strictly increasing'))).toBeTruthy()
  })

  it('accepts strict sequence', () => {
    const events = [
      UXEventSchema.parse({
        id: 'event-1',
        type: 'ux.action.proposed',
...
describe('StateProjectionSchema', () => {
  it('rejects stale lastVerifiedVersion', () => {
    ...
    expect(result.success).toBe(false)
    expect(JSON.stringify(result.error?.issues)).toContain('lastVerifiedVersion')
  })

  it('accepts consistent projection', () => {
    const result = StateProjectionSchema.safeParse({
      tenantId: 'tenant-1',
      siteId: 'site-1',
      version: 7,
...
describe('Transaction group validation', () => {
  it('rejects cross-mission grouping', () => {
    ...
    expect(result.passed).toBe(false)
    expect(result.errors.some((error) => error.includes('different tenant/session/mission'))).toBeTruthy()
  })
})

describe('TransactionGroupSchema', () => {
  it('accepts grouped transaction', () => {
...
describe('findDuplicateIdempotencyKeys', () => {
  it('duplicate idempotency keys are detectable', () => {
    ...
    expect(duplicates).toEqual([idempotencyKey])
  })
})
